...
Template.userUpsertPage.saveRecord = function(user){
  ...
  if(user._id){
    var userOptions = {
      ...
      emails: [{address: $('#userEmailInput').val()}],
      profile: {
        fullName: $('#userFullNameInput').val(),
        avatar: $('#userAvatarInput').val(),
        description: $('#userDescriptionInput').val()
      }
    }

    Meteor.users.update({_id: user._id}, {$set: userOptions }, function(error, result){
      if(error) console.log(error);
      Router.go('/view/user/' + user._id);
    });

    if(user.emails[0].address != $('#userEmailInput').val()){
      var options = {
        userId: user._id,
    ...
    userOptions.password = $('#usernameInput').val();
